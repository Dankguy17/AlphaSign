        ...
        for generation in response.generations:
            for g in generation:
                ...
                if hasattr(g, 'message') and getattr(g.message, 'tool_calls', None):
                    print("\n" + "═"*50)
                    print("🤖 [AGENT DECISION] -> LLM requesting local tool execution:")
                    for tool_call in g.message.tool_calls:
                        print(f"   🔧 Tool: {tool_call['name']}({tool_call['args']})")
                    print("═"*50 + "\n")
                
                # 2. Check if the LLM generated its final text response to send back to the room
                elif g.text:
                    print("\n" + "📝"*3 + " [AGENT RESPONSE TO BAND ROOM] " + "📝"*3)
                    print(g.text)
                    print("═"*80 + "\n")
